...
async function salvarStatusDoModal(botao) {
  ...
  if (emAprovacao) {
    ...
    if (decisao === 'aprovar') {
      ...
      if (dataPrev && dataPrev.value) payload.dataPrevistaConclusao = dataPrev.value;
    }
  ...
  const dataAgend = document.getElementById('mdlDataAgendamentoVisita');
  const valorAgendAtual = dataAgend ? dataAgend.value : '';
  const valorAgendOriginal = chamadoAtual.dataAgendamentoVisitaRaw || chamadoAtual.dataAgendamentoVisita || '';
  const agendamentoMudou = valorAgendAtual && valorAgendAtual !== gomFormatarInputDateModal_(valorAgendOriginal);
  if (agendamentoMudou) payload.dataAgendamentoVisita = valorAgendAtual;
  ...
  google.script.run
    .withSuccessHandler(function() {
      ...
      if (payload.dataAgendamentoVisita) camposLocais.dataAgendamentoVisita = payload.dataAgendamentoVisita;
...
async function salvarApenasObservacao(botao) {
  ...
  const dataAgend = document.getElementById('mdlDataAgendamentoVisita');
  const valorAgendObs = dataAgend ? dataAgend.value : '';
  const chamadoAtual = (window.listaChamadosGlobal || []).find(function(x) { return String(x.id) === String(idChamadoAberto); }) || {};
  const valorAgendOriginalObs = chamadoAtual.dataAgendamentoVisitaRaw || chamadoAtual.dataAgendamentoVisita || '';
  // Só considera mudança real se o valor é diferente do que já está salvo
  const agendamentoMudouObs = valorAgendObs && valorAgendObs !== gomFormatarInputDateModal_(valorAgendOriginalObs);
  ...
  if (agendamentoMudouObs) payload.dataAgendamentoVisita = valorAgendObs;
  if (anexosAtualizacao.length) payload.anexosAtualizacao = anexosAtualizacao;
  ...
  google.script.run
    .withSuccessHandler(function() {
      ...
      const camposLocais = {};
      if (obs) camposLocais.observacoes = (chamadoAtual.observacoes ? chamadoAtual.observacoes + '\n' : '') + obs;
      if (agendamentoMudouObs) camposLocais.dataAgendamentoVisita = valorAgendObs;
