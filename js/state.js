...
function iniciarSistema() {
  try {
    carregarUsuarioPermissoes(function(usuario) {
      ...
      var paginaSalva = '';
      try {
        paginaSalva = typeof gomLerRotaSalva === 'function'
          ? gomLerRotaSalva()
          : (sessionStorage.getItem('gom:lastPage') || localStorage.getItem('gom:lastPage') || '');
      } catch(e) {}
      var podePaginaSalva = paginaSalva && typeof podeAcessarPagina === 'function' && podeAcessarPagina(paginaSalva);
      // Restauração obrigatória de rota: se o usuário estava em Fila, Empresa,
      // Campo ou em uma subaba, a atualização da página mantém o mesmo módulo.
      // O Dashboard só é usado quando não existe rota válida salva ou quando o
      // usuário clica explicitamente em Secretaria/Dashboard.
      const paginaInicial = podePaginaSalva
        ? paginaSalva
        : getPaginaInicialPermitida('dashboard');
